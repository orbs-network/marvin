...
func UnmarshalAccounts(bytes []byte) (map[string]*Key, error) {
	accounts := make(map[string]*Key)
	err := json.Unmarshal(bytes, &accounts)
	return accounts, err
}

const TestAccountsFilename = "accounts.json"
...
var ed25519KeyPairs = []ed25519KeyPairHex{
	...
	{"A19E8DfCeD9ACcb96b336dC4a4ab863Ac70E5Bc9bfa89aA88FbEd269b3A378eA", "5C34713a6f0c3804d5BaB5372deff4ddAd2Aa3eeDef7567aa5C97371D3DaCe19A19E8dfced9aCCB96b336DC4a4ab863aC70E5bc9Bfa89aA88FBED269B3A378Ea"},
...
func readAccounts(filename string) map[string]*RawKey {
	if !util.DoesFileExist(filename) {
		util.Die("Input file %s does not exist", filename)
	}

	bytes, err := ioutil.ReadFile(filename)
	if err != nil {
		util.Die("Could not open accounts file '%s'.\n\n%s", filename, err.Error())
	}

	accounts, err := UnmarshalAccounts(bytes)
	if err != nil {
		util.Die("Failed parsing accounts json file '%s'. Try deleting the key file to have it automatically recreated.\n\n%s", filename, err.Error())
	}
	...
	for _, key := range accounts {
		...
		if err != nil {
			util.Die("Could not parse hex string '%s'. Try deleting the key file '%s' to have it automatically recreated.\n\n%s", privateKey, filename, err.Error())
		}
...
func ReadAccountsFromFile(filename string) [][]byte {
